...
!END-PKSTRAT
    ...
    !US standard atmosphere params
      integer, parameter :: nstd=15 ! # of levels specified in US standard atmosphere
...
!      real(r8) :: lapse(nstd) ! Lapse rate of standard levels 
      ...
      real(r8) :: pstd_norm(nstd) = & ! standard pressure (Pa)
      ...
      real(r8) :: tstd(nstd) = & ! standard temperature (K)
      ...
      real(r8) :: lapse(nstd) = & ! temperature lapse rate (K/m)
...
!      lapse=(/ -0.0065, 0.0, 0.001, 0.0028, 0.0, -0.0028, -0.002 /)
    ...
    !END-PKSTRAT
      ...
      pstd_norm(:) = pstd_norm(:)/pstd_norm(1)
